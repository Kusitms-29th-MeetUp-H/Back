...
public interface SyncRepository extends JpaRepository<Sync, Long> {
    ...
    @Query("SELECT s FROM Sync s WHERE s.syncType = :syncType AND s.user.id = :userId")
    List<Sync> findAllByUserIdAndSyncType(@Param("userId") Long userId, @Param("syncType") SyncType syncType);
}
